...
- Sección wtf EP2 (el streamer mueve/resizea todo mid-live): la cámara alterna 2 geometrías — STD `(1632,48,1869,393)` y BIG `(1498,308,1870,776)` — con switches medidos por clip (`tools/trackcam.py`: detecta borde azul H 85-112 + bisect 1s; crop BIG `left=1498 top=308 right=51 bottom=304`, mismo slot PiP). V1 = crop al ÁREA DE VIDEO medido por clip + `distort=0` (los videos verticales llenan, los apaisados quedan con bandas como reel normal); segmentos browse/grilla = crop central 9:16. Si la webcam se oculta (alerta fullscreen) se termina V2 ahí y V1 sigue solo. `tools/mkshotcut.py` genera los `.mlt` (misma estructura + hash MD5 primer+último MB de `media.py`) y se validan con contact sheet.
