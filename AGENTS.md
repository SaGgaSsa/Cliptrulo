...
- Escenas con layout distinto (ver `detect_scenes.py`, clase REEL/GRID/FULL a 1fps + `scenes.json`): V1 se parte por escena, cada parte con su crop + affine full-bleed `distort=1`. Clip_01 EP2: GRID (perfil/grilla) crop `left=320 top=50 right=290 bottom=50` (320 excluye badge RELATANDO y filo izquierdo); REEL (modal video+comentarios) crop `left=515 top=60 right=890 bottom=70` (solo panel de video, sin sidebar); V2-cam = standard. Título CHITRULO sale en el PiP (debajo de y=48): es parte de la receta standard, se deja.
