...
- Posición STANDARD EP2 (layout fijo: video vertical + cam con título; recalcular por video si cambia): V1 crop `left=660 top=0 right=660 bottom=35` + affine `0 0 1080 1920` con `distort=1` (el video llena todo el 9:16 aunque estire); V2-cam crop `left=1632 top=48 right=51 bottom=687` (título CHITRULO + caja al borde cyan, sin gris) + affine `760 20 300 437` con `distort=0` (PiP arriba-derecha).
