...
0. Asegurar binario: `python tools/ensure_chamu_cli.py` → imprime `CHAMU_CLI=<ruta>` (`UP-TO-DATE` / `DOWNLOADED` / `LOCAL-FALLBACK`; exit 2 = correr `gh auth login`). El exe vive en `tools/chamu-cli/` (no se commitea, solo `VERSION.json`). Usar esa ruta en el paso 3.
...
3. Audio + transcribir por sección: wav 16kHz mono, luego `<CHAMU_CLI> --model small --language es --format json --time-offset 0 <ID>.wav > <ID>_chamu.json` (exit 0 = ok; 3 = sin voz; 30 min máximo por archivo).
